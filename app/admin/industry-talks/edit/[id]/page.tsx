...
// Converts a raw seconds value (as returned by the API) into "mm:ss" for the
// duration input. If the value is already a "mm:ss"-style string, it's
// returned as-is.
function formatDurationForInput(value: unknown): string {
...
export default function EditIndustryTalkPage() {
  ...
  async function submit(status: "DRAFT" | "PUBLISHED") {
    if (!id) {
      setMessage("Missing interview id")
      return
    }
    if (!form.title.trim()) {
    ...
    // Same reasoning as the create page: the backend's upload middleware
    // only parses `multipart/form-data`, so we send FormData, not JSON.
    const fd = new FormData()
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-900">
                Edit CEO Interview / Industry Talk
              </h1>
              <p className="text-xs text-gray-400">
                Dashboard &gt; CEO Interviews &gt; Edit Interview
              </p>
            </div>
          </div>
